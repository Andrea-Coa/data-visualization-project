...
\begin{itemize}
    ...
    \item \textbf{Flujo de procesamiento esperado:} dos pipelines paralelos (crudo $\rightarrow$ listo) que confluyen en un \emph{merge} final. Verificamos que las ROI de \citet{cui2026} (frontal: 16 electrodos; centro-temporal: 28) están completamente contenidas en el montaje de 61 canales de ds004902.
    \begin{itemize}
        \item[--] \emph{EEG (señal):} Crudo (BIDS \texttt{.set}/\texttt{.fdt}, 61 canales, 500 Hz) $\rightarrow$ \textbf{limpieza} (canales malos, filtro pasa-banda 1--40 Hz, notch 50 Hz) $\rightarrow$ \textbf{segmentación} (75 épocas de 4 s por grabación, siguiendo a \citealt{cui2026}) $\rightarrow$ \textbf{extracción} (PSD por época/electrodo, potencia relativa theta) $\rightarrow$ \textbf{agregación} (media y variabilidad entre épocas, por sujeto/condición/electrodo y por ROI) $\rightarrow$ JSON.
        \item[--] \emph{Conductual (tabular):} Crudo (\texttt{participants.tsv}) $\rightarrow$ \textbf{limpieza} (parseo de \texttt{n/a}, tipado) $\rightarrow$ \textbf{filtrado} (solo sujetos con dato pareado NS+SD, $n$ documentado por variable) $\rightarrow$ \textbf{derivación} ($\Delta$PVT, $\Delta$KSS, $\Delta$PANAS-PA $=$ SD $-$ NS) $\rightarrow$ CSV.
        \item[--] \emph{Merge:} \emph{join} por \texttt{participant\_id} entre el resumen EEG y las deltas conductuales $\rightarrow$ tabla lista para las tres vistas D3.js. Los archivos EEG crudos ($\sim$8 GB vía S3) no se procesan en el navegador.
    \end{itemize}
\end{itemize}
}
...
\begin{enumerate}[leftmargin=2.2em, label=\textbf{Q\arabic*.}, ref=Q\arabic*]
    ...
    \item \textbf{(Variabilidad vs.\ media)} ¿La variabilidad de potencia theta entre épocas revela patrones espaciales de vulnerabilidad más amplios o distintos que los que revela la potencia media theta por banda?
    \item \textbf{(Panorama de vulnerabilidad individual)} ¿Qué tanto se distinguen los sujetos vulnerables de los resistentes según su cambio de variabilidad theta (frontal/centro-temporal) y su cambio en tiempo de reacción del PVT?
    \item \textbf{(Firma espacial de la vulnerabilidad)} ¿Los sujetos vulnerables comparten, de forma consistente entre ellos, una firma espacial de variabilidad theta concentrada en las regiones frontal y centro-temporal?
